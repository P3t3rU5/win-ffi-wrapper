...
module WinFFIWrapper
  class Window #< Control
    ...
    bindable :title,
             ...
             setter: (->(value) do
               set_value(:title, value) do
                 User32.SetWindowTextW(hwnd, value.to_w)
               end
             end)
    ...
    def initialize(title:  '',
      ...
      self.title = title
      Dialog.message_box(title, :ICONERROR)
      Dialog.message_box(self.title, :ICONERROR)
      ...
      @title = title
      @hwnd = User32.CreateWindowExA(style.create_style_ex, #WindowStyleExEnum
      ...
      # Dialog.message_box(get_title.to_w, :ICONERROR) if @hwnd
    ...
    def get_title
      title_lalala = nil
      title_size = User32.GetWindowTextLengthW(@hwnd) + 1
      FFI::MemoryPointer.new(:ushort, title_size) do |title|
        User32.GetWindowTextW(@handle, title, title_size)
        title_lalala = title ? title.read_array_of_uint16(title_size - 1).pack('U*') : ''
      end
      title_lalala
    end
